...
class GoalWrapper(Wrapper):
    ...
    def step(self, action):
        ...
        if self.env._maze_id == "Key":
            key_distance = np.linalg.norm(observation[..., :self.goal_dim] - np.array([8,4]), axis=-1)
            if key_distance < self.distance_threshold:
                self.get_key = True
            else:
                self.get_key = False
            info['is_success'] = info['is_success'] and self.get_key
    ...
    def reset(self):
        ...
        if self.random_start:
            if self.env._maze_id == "Push":
                ...
                while  (xy[0] >= 2) or (xy[1] >= 6) or (self.env._is_in_collision(xy)):
                    xy = self.maze_space.sample()
            else:
                xy = self.maze_space.sample()
