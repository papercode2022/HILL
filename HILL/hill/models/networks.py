...
class actor(nn.Module):
    def __init__(self, env_params, goal_dim, device):
        super(actor, self).__init__()
        self.max_action = torch.tensor(env_params['action_max'], dtype=torch.float32).to(device)
...
class critic(nn.Module):
    def __init__(self, env_params, args, goal_dim):
        ...
        self.max_action = torch.tensor(env_params['action_max'], dtype=torch.float32).to(args.device)
